...
myboot2<-function(iter=10000,x,fun="mean",alpha=0.05,cx=1.5,...){  #Notice where the ... is repeated in the code
  ...
  y=sample(x,n*iter,replace=TRUE) #  Makes another sample
  rs.mat=matrix(y,nr=n,nc=iter,byrow=TRUE) # n rows by iter columns
  xstat=apply(rs.mat,2,fun) # Finds "fun" of every column - xstat is a vector and will have iter values in it
